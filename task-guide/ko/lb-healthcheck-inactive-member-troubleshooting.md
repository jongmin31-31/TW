...
- **멤버 등록 전 사전 점검**: Load Balancer에 멤버를 등록하기 전에, 멤버 인스턴스에 적용된 보안 그룹이 `default`가 아니라면 Load Balancer 트래픽을 허용하는 규칙이 미리 추가되어 있는지 확인합니다. 또한 Load Balancer에 IP 접근 제어 그룹이 설정되어 있다면, 새로 등록할 멤버의 IP가 차단 대상에 포함되어 있지 않은지 함께 확인합니다.
