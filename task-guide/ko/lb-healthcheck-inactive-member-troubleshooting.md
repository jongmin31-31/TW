...
Load Balancer에 등록한 멤버(Load Balancer가 트래픽을 분산하는 대상 인스턴스)의 상태가 `INACTIVE`로 표시되고, 해당 멤버로 트래픽이 전달되지 않습니다.
...
> Load Balancer에 **IP 접근 제어 그룹**을 별도로 설정한 경우에는, 원인 2도 함께 확인합니다. IP 접근 제어 그룹을 설정하지 않았다면(콘솔에 `-`로 표시됨) 이 원인은 해당하지 않습니다.
...
IP 접근 제어 타입이 **허용**으로 설정된 경우에도 같은 문제가 발생할 수 있습니다. 허용 타입은 등록된 IP만 통신을 허용하고 그 외 모든 IP는 차단하는 방식이므로, 등록한 목록에 멤버 인스턴스의 IP가 빠져 있으면 멤버가 보내는 응답(헬스 체크 응답 포함)이 차단되어 `INACTIVE`로 전환됩니다. 실무에서는 허용 정책을 적용할 때 외부에서 접근하는 클라이언트 IP 대역만 등록하고 Load Balancer에 할당된 멤버 인스턴스의 IP는 등록하지 않는 경우가 많아 이 문제가 자주 발생합니다.
...
6. IP 접근제어 타입이 **허용**인 경우, 이 목록에 멤버 인스턴스의 IP가 포함되어 있는지 확인합니다. 외부 클라이언트 IP 대역만 등록하고 멤버 인스턴스의 IP는 빠뜨리는 경우가 많으니 반드시 함께 확인합니다.
...
> **⚠️ 주의**
> 허용 타입은 등록된 IP만 통신을 허용하는 방식이므로, 외부에서 접근하는 클라이언트 IP 대역만 등록하고 Load Balancer에 할당된 멤버 인스턴스의 IP는 등록하지 않으면 헬스 체크가 실패해 멤버가 `INACTIVE`로 전환됩니다. 허용 정책을 적용할 때는 클라이언트 IP 대역과 함께 멤버 인스턴스의 IP도 반드시 등록해야 합니다.
...
- **멤버 등록 전 사전 점검**: Load Balancer에 멤버를 등록하기 전에, 멤버 인스턴스에 적용된 보안 그룹이 `default`가 아니라면 Load Balancer 트래픽을 허용하는 규칙이 미리 추가되어 있는지 확인합니다. 또한 Load Balancer에 IP 접근 제어 그룹이 설정되어 있다면, IP 접근제어 타입이 **차단**인 경우 새로 등록할 멤버의 IP가 차단 대상에 포함되어 있지 않은지, **허용**인 경우 멤버의 IP가 허용 대상에 포함되어 있는지 함께 확인합니다.
